...
export default function RegisterConfirmForm({ submit }: RegisterConfirmFormProps) {
  ...
  return (
    <Box component='form' onSubmit={formik.handleSubmit}>
      ...
      <Box sx={{ mt: 2, mb: 0 }}>
        <TextField fullWidth type='text' formik={formik} name='confirmCode' label={t('login.confirmation_code')} />
      </Box>
      <Button
        fullWidth
        variant='contained'
      ...
      <Typography sx={{ display: 'flex', justifyContent: 'end', mt: 2, mb: 1 }} variant='body2'>
        <Button onClick={handleResendConfirmation} size='small' variant='outlined' type='button' disabled={disabled}>
          {disabled ? `${t('login.wait_for_resend')} ${time}` : t('login.send_code_again')}
        </Button>
      </Typography>
    </Box>
  )
