...
export default function Login({ step }: LoginProps) {
  ...
  const signUpSubmit = async ({ email, password, given_name, family_name }: UserAttributes) => {
    signUp(
      {
        email,
        password,
  ...
  const confirmSignUpSubmit = async ({ confirmCode }: ConfirmCode) => {
    confirmSignUp({ confirmCode }, userEmail, navigate, enqueueSnackbar, t, dispatch)
  }

  const signInSubmit = ({ email, password }: LoginValues, setSubmitting: SetSubmitting) => {
    signIn({ email, password }, dispatch, navigate, enqueueSnackbar, t, setSubmitting)
  }
    ...
    <Grid container spacing={2}>
      ...
      >
        ...
        >
          ...
          {step === 'confirmationCode' && <RegisterConfirmForm submit={confirmSignUpSubmit} />}
